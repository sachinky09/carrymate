...
export async function createRequest(req, res, next) {
  try {
    const { source, destination, item_name, item_description, weight, needed_by } = req.body;

    if (!source || !destination || !item_name || !weight || !needed_by) {
      throw new AppError('source, destination, item_name, weight and needed_by are required', 400, FILE);
    }
    if (!involvesNitDurgapur(source, destination)) {
      throw new AppError('Every request must involve NIT Durgapur', 400, FILE);
    }
    ...
    if (req.file) {
      const imagePath = await uploadRequestImage(request.id, req.file.buffer, req.file.mimetype);
      const { error: updateError } = await supabase
        .from('requests')
        .update({ image_url: imagePath })
        .eq('id', request.id);
      if (updateError) throw new AppError(updateError.message, 500, FILE);
...
export async function getMyRequests(req, res, next) {
  try {
    ...
    const withSignedUrls = await Promise.all(
      data.map(async (r) => ({
        ...r,
        image_signed_url: r.image_url ? await getSignedImageUrl(r.image_url) : ''
      }))
    );
...
export async function cancelRequest(req, res, next) {
  try {
    ...
    if (fetchError || !request) throw new AppError('Request not found', 404, FILE);
    if (request.sender_id !== req.user.id) throw new AppError('You do not own this request', 403, FILE);
    if (request.status !== 'pending') throw new AppError('Only pending requests can be cancelled', 400, FILE);
